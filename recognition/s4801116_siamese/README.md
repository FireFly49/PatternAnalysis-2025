...
(benign and malignant) with a goal accuracy of 80%. Additionally, 
as a secondary goal, there was a greater emphasis of making sure that the
malignant labels were predicted more often, as this dataset
is largely imbalanced and the regular accuracy metrics
were easily skewed and it is more useful in medical setting.
...
file and placed into the `data/processed/` folder into two separate folders
for each class.
...
oversampling was also attempted, however this lead to a more clear
trend of overfitting and was hence discarded for this version.
...
skin tone, perspective or hair follicles covering the area
...
A separate network was implemented to use the embeddings
...
1. Loss Function: Triplet Margin Loss
2. Optimizer: Adam
3. Hyperparameters and other configuration
   parameters: Refer to `config.py`

## Results

Graphs made for analysis will be stored under the `metrics` folder after running the `predict.py` script
...
accuracy of 80% was met. However, this occurred during
...
generate meaningful improvements throughout the epochs and hence
...
the model is overfitting, more specifically the siamese
...
From the confusion matrix above, we can see that from the validation
batch with class proportions: (6509, 117) (Benign, Malignant). 
These proportions were retrieved after running `dataset.py`.
The majority of malignant targets were predicted correctly, which 
was the secondary objective of the project. Benign labels were
also mostly classified correctly across the model. False positives
(false alerts on malignant lesions) are not as critical as 
False negatives so, the higher false positive rate is preferred 
to having a higher false negative rate.

#### Training set

![Alt text](metrics/confusion_matrix_train.png)
Somewhat contrary to the previous findings, the training
confusion matrix seems to prefer the overall accuracy 
more than reducing false negatives; there are more false negatives
in this model than false positives which is a significant issue
for overall generalisation over time. This further confirms the 
overfitting suspicion previously stated
...
the feature separation between the two target labels 
for the skin lesions. This can be seen to mostly be 
well separated, and the goal of reducing 
...
were classified correctly as seen in the feature separation and the confusion matrix; 
